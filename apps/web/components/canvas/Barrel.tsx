...
export const Barrel = () => {
    const { length, maxDiameter, cuts, frontTaperLength, rearTaperLength, holeDepthFront, holeDepthRear, outline, outlineInterp, frontEndShape, rearEndShape, polygonSides, activeCutId } = useBarrelStore();

    const geometry = useMemo(() => {
        const geom = generateBarrelGeometry(length, maxDiameter, cuts, frontTaperLength, rearTaperLength, holeDepthFront, holeDepthRear, outline, frontEndShape, rearEndShape, outlineInterp, polygonSides);
        geom.rotateX(-Math.PI / 2);
        geom.translate(0, 0, -length / 2);
        return geom;
    }, [length, maxDiameter, cuts, frontTaperLength, rearTaperLength, holeDepthFront, holeDepthRear, outline, outlineInterp, frontEndShape, rearEndShape, polygonSides]);
